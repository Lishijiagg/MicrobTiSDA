test_that("Data.cluster.cut", {
  ...
  oslo_clust_results = Data.cluster.cut(cluster_outputs = oslo_model_clust,
                                        cut_height = 0.15,
                                        font_size = 2.5)
  expect_true(oslo_clust_results$auto_cutree == FALSE)
  expect_s3_class(oslo_clust_results,"MicrobTiSDA.clusterCut")
  expect_true(length(oslo_clust_results$cluster_results) == length(unique(oslo_meta_int$Group)))
})
