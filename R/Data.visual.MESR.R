...
#' @examples
#' \dontrun{
#' # Assuming you have hierarchical clustering results (cluster_results), predicted data (predicted_data),
#' # design data (Design_data), pre-processed microbial data (pre_processed_data), and optional taxonomy annotations (Taxa):
    ...
    for (j in as.numeric(levels(as.factor(cutrees)))) {
      ...
      if (plot_dots == TRUE) {
        ...
        point_data_for_plot_long = point_data_for_plot %>%
          pivot_longer(cols = -c(Time,ID),names_to = "Features",values_to = "Abundance")
        curves_plot[[i]][[j]] = curves_plot[[i]][[j]]+
