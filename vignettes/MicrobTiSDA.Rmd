...
First we have to lad the installed package and other necessary packages in the workspace with
...
We first filter out OTUs from the OTU table that have a total abundance of less than 5000 across all samples and a prevalence of less than 20% in all samples from each subject.
...
Next, we will infer the species interactions within the gut microbiomes of each individual and visualize the topological results of these species interactions. To reduce computation time, we will set the number of iterations for bagging to 5 (the default is 10).
...
# Visualize the species interaction results of ID10
...
By calculating the correlation distances between the time series of microbial features predicted by the model, we will employ hierarchical clustering to identify OTU clusters with similar temporal patterns. If the auto_cutree parameter of the Data.cluster function is set to FALSE, a dendrogram of the microbial feature time series clusters for each subject will be output during the execution of the Data.cluster function. The user will need to manually input the correlation distance threshold for OTU clustering. In this case, we will set the threshold for clustering OTUs to a correlation distance of less than 0.15.
...
# For example, taking OTU_000001 of ID10, we can observe microbial features with a correlation distance greater than 0.9
...
preterm_rf$`cross-validation`
```

Here, we choose to use 70% of the samples as the training set and the remaining 30% as the testing set. Given that the number of microbial features in this dataset is relatively small, we didn't filter out low_abundant microbial features in this step (`OTU_counts_filter_value = 0`). The results indicate that the sample classification model built using all OTUs as inputs demonstrates high performance, with an Out of Bag error of 0.081. Additionally, based on the cross-validation curve, we selected the top 125 OTUs as biomarkers according to their mean decrease accuracy (MDA) ranking.

Using these biomarkers and applying NMDS (Non-metric Multidimensional Scaling), we can observe that the selected biomarkers effectively distinguish between the two groups of samples.
...
Here, OTUs with a correlation distance of less than 0.2 are grouped into clusters.
