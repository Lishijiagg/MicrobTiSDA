---
title: "MicrobTiSDA-vigenette"
output: rmarkdown::html_vignette
eval: false  
vignette: >
  %\VignetteIndexEntry{MicrobTiSDA-vigenette}
...
By calculating the correlation distances between the time series of microbial features predicted by the model, MicrobTiSDA employs hierarchical clustering to identify OTU clusters with similar temporal patterns. If the `auto_cutree` parameter of the Data.cluster function is set to FALSE, a dendrogram of the microbial feature time series clusters for each subject will be output during the execution of the `Data.cluster` function. The user will need to manually input the correlation distance threshold for OTU clustering. In this case study, we will set the threshold for clustering OTUs to a correlation distance of less than 0.15.
